...
| confidenceSignals     | boolean<br/>or<br/>`"strict"`<br/>or<br/>object<br/>default: `true`            | If true, Sensible prompts the LLM to report any uncertainties it has about the accuracy of its response.  For more information, see [Qualifying LLM accuracy](doc:confidence).<br/>If `"strict"`, Sensible returns null for a field if its confidence signal is `incorrect_answer`.<br/>To configure both options, or to pin the LLM engine used for confidence signals independently of the engine used for extraction, specify an object with one or both of the following properties:<br/>- `engine`: the LLM provider to use for confidence signals. `"open-ai"`, `"anthropic"`, or `"google"`.<br/>- `strict`: boolean. If true, Sensible returns null for a field if its confidence signal is `incorrect_answer`.<br/><br/>At least one property must be present. For example: `{ "engine": "open-ai" }` or `{ "engine": "google", "strict": true }`. | Not supported if you specify the Multimodal Engine parameter or Source Ids parameter |
|                       |                                                              | ***FIND CONTEXT***                                           |                                                              |
