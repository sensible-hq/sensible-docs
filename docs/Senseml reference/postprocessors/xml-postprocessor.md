...
| rule (**required**) | JsonLogic object          | Define the XML output using a [JsonLogic](doc:jsonlogic) rule. See [Rule parameters](#rule-parameters) for the rule syntax. The rule must evaluate to a root element object. To generate one XML element per extracted field without naming each field individually in the rule, see [Example 2.]() |
