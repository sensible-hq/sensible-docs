...
| selfCloseEmptyTags  | boolean. default: `true`  | If true, Sensible renders elements with no content as self-closing tags (for example, `<null_field/>`). If false, Sensible renders them as an open-and-close tag pair (for example, `<null_fiel></null_field>`).  Fields with null output produce elements with no content. |
