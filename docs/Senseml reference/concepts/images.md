...
| Search for non-labeled, non-text images in a range. For example, search for unlabeled photos of houses in a real estate document. This option returns images' coordinates, which you can then use to render the images.                                                                               | Use the [Document Range](doc:document-range) method with the Include Images parameter configured. |
