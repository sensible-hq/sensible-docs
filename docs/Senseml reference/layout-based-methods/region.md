...
| asImage                | boolean. default: `false`         | When true, Sensible returns the region as an image. Sensible returns the image's coordinates and its base64-encoded string, for example,  `data:image/png;base64data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAABMgAAABaCAYAAA....` Use this option to capture visual content instead of extracting text. <br>For alternatives to this parameter, see [Image processing](doc:images). |
