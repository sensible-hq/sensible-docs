...
next:
  description: ''
---
Searches for a checkbox near to the anchor and returns a boolean indicating if it's selected or unselected.
...
| ignoreFormData    | boolean. default: false         | Set this option to true to troubleshoot situations in which Sensible fails to recognize a checkbox because of a document's inaccurate checkbox metadata. For example, a PDF editor can run partially successful form recognition on a scanned document and embed incomplete form data in the PDF.  When true, this option bypasses Sensible's default use of checkbox metadata and falls back to pixel recognition. |
