...
| fields                   | array of objects that can be of the following types:<br/>-  [Computed field](doc:computed-field-methods)<br/>-  [Conditional method](doc:conditional)<br/>-  [Custom Computation Group field](doc:custom-computation-group) | Specifies to output computed fields in the new table. The computed fields have access to the source table's fields.<br/>To get access to and transform the output of fields that aren't in the source table, use the [Copy to section](doc:copy-to-section) method. |
