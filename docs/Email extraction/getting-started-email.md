...
3. Optionally, Sensible can treat an attachment as a [portfolio](doc:portfolio) — a single file containing multiple documents — and extract each one separately. For example, an applicant might attach a single combined PDF containing both a paystub and a signed lease. A single email can have some attachments handled as portfolios and others as single documents. When you contact Sensible to create a processor, indicate which attachments may arrive as combined files and provide the document types for Sensible to segment them into.
