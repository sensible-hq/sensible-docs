...
3. Optionally, Sensible can treat an attachment as a [portfolio](doc:portfolio) — a single file containing multiple documents — and extract each one separately. For example, an applicant might attach a single combined PDF containing both a paystub and a signed lease. You configure this per attachment type when you set up the processor, so a single email can have some attachments handled as portfolios and others as single documents. When you contact Sensible to create a processor, indicate which types of attachments may arrive as combined files.
