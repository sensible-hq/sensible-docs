...
1. Classify a document by its similarity to high-level document types you define in your Sensible account. For example, classify a document as a `1040s` document type or a `pay_stubs` document type. You can classify a document by type using the Sensible API's [Classify](reference:classify-document)  endpoint.
