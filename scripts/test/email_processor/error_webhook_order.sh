#!/bin/bash
# Expects: 400 — EnvironmentWebhook must not be first in the array.
# The spec notes that schema validators cannot catch this; only the API can.
mkdir -p "$(dirname "$0")/outputs"
OUTFILE="$(dirname "$0")/outputs/error_webhook_order_$(date +%Y-%m-%d_%H-%M-%S).json"
RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT \
  ...
  -d '{
    ...
    "webhooks": [
      {
        "url": "https://webhook.site/dev-b9dad6da-5902-43d8-bca9-423347f000f0",
