...
&#x20;If you want to call the Sensible API using the MCP server, configure an authorization header for the server using your Sensible API key. The server doesn't support OAuth flows. For example, in Claude Code, add the following entry to `~/.claude.json`, and specify the `SENSIBLE_API_KEY` variable in your environment using the value of your [API key](https://app.sensible.so/account/?t=api_keys):
