...
| currencySymbol            | string or object. Default: `$`                               | The currency symbols to recognize, for example "€" or "EURO". The symbol must precede the amount. This parameter sets the Unit parameter in the output.<br/>To specify multiple symbols to recognize, create a lookup table. The table can optionally map source text to the Unit parameter.  For example, the following lookup table recognizes the dollar code and the euro code, and outputs the codes as symbols to Unit parameter:<br/> `"currencySymbol": {`<br/>           `USD: "$",`<br/>           `EUR: "€",`<br/>           `default: "EUR"}`<br/>If the source text doesn't include a currency symbol, Sensible uses the default specified in the lookup table. If the lookup table doesn't include a default, Sensible falls back to the $ symbol. |
