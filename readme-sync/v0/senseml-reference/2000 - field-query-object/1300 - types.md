...
| -------------------- | --------------------- | ------------------------------------------------------------ |
| id (**required**)    | `compose`             |                                                              |
| types (**required**) | array of type objects | Each type in a compose array takes the output of the previous type as its input. Note that Sensible recommends explicitly configuring a  [tiebreaker](doc:method#parameters) on the field if you configure this type for a field that can output an array. The tiebreaker applies to the output of the last type in the array. |
