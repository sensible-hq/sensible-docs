...
| replaceWith (**required**) | string                                 | Specifies the text with which to replace each match. For example, replace every digit in a string (`"pattern:"[0-9]"`) with the character "x" (`"replaceWith":"x"`). For a string `a12bc3` this returns `axxbx`. <br/>Or, specifies to replace capturing groups. For example, `"pattern": "(account)\\s(number)"` and `"replaceWith": "$2, $1"` returns `number, account`. |
