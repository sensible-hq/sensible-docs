...
| :--------- | :---- | :----------------------------------------------------------- |
| id         | `zip` |                                                              |
| source_ids | array | The ids of the fields to zip together, or a single id for field that returns a table. If the output of the source IDs are arrays (for example, if you specify `"match: all"`), the Zip method joins them up to their maximum shared length. For example, if you zip arrays that have 4, 5, and 6 elements respectively, the zipped array has 4 elements. If you mix array source IDs with one or more table source IDs, the Zip method zips the first table and ignore everything else (tables take precedence over arrays for zipping). |
...
![Click to enlarge](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/images/final/zip_example.png)


Try out this example in the Sensible app using the following PDF and config:

| Example PDF | [Download link](https://raw.githubusercontent.com/sensible-hq/sensible-docs/main/readme-sync/assets/v0/pdfs/zip_example.pdf) |
| ------------------- | ------------------------------------------------------------ |
