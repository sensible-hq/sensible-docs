...
In this case, it is best practice to add each document to the appropriate document type, rather than trying to fit them all into one document type (which would break any [validations](doc:validate-extractions) you write for the doc type). For example, if you already have a tax doc type and an invoice doc type, use configs in these preexisting doc types rather than creating a "combined_tax_and_invoice" doc type.  In order for Sensible to handle the document inside the portfolio, specify the following:
