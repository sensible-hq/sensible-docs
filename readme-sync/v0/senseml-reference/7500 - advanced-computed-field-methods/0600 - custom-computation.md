...
| :----------------------- | :----------------------------------------- | :----------------------------------------------------------- |
| id (**required**)        | `customComputation`                        | This method has access to the  `parsed_document` as follows: <br/>- where [verbosity](doc:verbosity) = 0<br/>- at a depth of 1 section in the case of nested sections<br/><br/>This method operates on anything that is of the types ``"string", "number", "boolean", "null"`  or an array of those. It returns the same types. |
| jsonLogic (**required**) | [JsonLogic](https://jsonlogic.com/)  rules | Outputs a computed field by using Boolean, logic, numeric, array, string, and other operations on extracted fields. Supports all [JsonLogic operations](https://jsonlogic.com/operations.html) and extends them with Sensible operations such as `"exists"` and `"match`". For more information about Sensible operations, see  [Validating extractions](doc:validate-extractions#parameters).<br/><br/>Double escape any dots in the field keys (for example for a field ID `delivery.zip.code`, escape as `delivery\\.zip\\.code`). |
