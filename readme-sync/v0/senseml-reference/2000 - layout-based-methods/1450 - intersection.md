...
| verticalAnchor<br/>or<br/>horizontalAnchor (**required**) | Anchor object                | An anchor object that defines an intersection with the field-level anchor. Sensible extracts lines that overlap to any extent with the intersection point of two perpendicular lines that each bisect one of the two anchors.<br/>The vertical and horizontal anchors can be on different pages. When horizontal and vertical anchors are on different pages, the horizontal anchor determines which page Sensible find the anchor on.<br/>To extract multiple intersections using one field, specify `"match":"all"` for the anchor at the field level. For examples, see examples 2 and 3. |
