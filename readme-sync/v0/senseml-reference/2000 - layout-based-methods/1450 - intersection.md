...
| percentOverlapY                                           | number. default: 0.8         | If you use the Width or Height parameters to extract lines contained in a region, then you can configure the strictness of the criteria by which a region "contains" a line using this parameter.<br/>By default, Sensible determines that a region contains a line if their heights overlap by more than 80%  of the smaller of the two's height. Loosen the criteria if a line can partly fall outside a region. For example, if you set this parameter to 0.4, then Sensible determines that a region contains a line if their heights overlap by more than 40%  of the smaller of the two's height. Note the line must also meet the Percent Overlap X parameter's criteria. |
