...
| requiredFields        | object                                                 | Array of field IDs that must be non-null for Sensible to return a section.<br/> In the [Claims loss run example](doc:sections-example-loss-run), add the following code to the sections group to omit claims that lack a phone number: <br/> `"requiredFields": ["phone_number"],`<br/>With the preceding code, Sensible omits claim number `9876543211`  from the output. |
