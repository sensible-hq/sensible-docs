...
| offsetY               | number in inches. Positive values offset down the page, negative values offset up the page. | Specifies the number of inches by which to offset each section's top boundary from the anchor's Match parameter. <br/>By default each section starts at the top boundary of the line specified by the anchor's Match parameter. If you specify Offset Y, each section starts at that top boundary plus the offset. For example, configure this when each section lacks an easy-to-match first line. | Specifies the number of inches by which to offset the columns' shared top boundary from the anchor's Match parameter. <br/>For example, configure this when when you want to exclude non-columnar text from columnar sections. |
...
| lineSelection         | Matcher[]                                                    |                                                              | TODO DID I MISS THIS ONE??                                   |
