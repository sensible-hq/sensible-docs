...
| pageHinting            | boolean. default: true                                       | Includes or or removes page metadata for each chunk from the prompt that Sensible inputs to the LLM.  For example, removes phrases like ``The excerpt starts at the top of page 1 and ends at the bottom of page 1.`` |
...
For each Sensible Instruct method, Sensible creates a full prompt for a large-language method (LLM) that includes your prompts and *context*, or chunks excepted from a document. For more information about configuring the full prompt that Sensible inputs to an LLM, see [Advanced prompt configuration](doc:prompt).
