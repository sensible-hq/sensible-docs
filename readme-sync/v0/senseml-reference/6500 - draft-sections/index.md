...
| columnSelection | array of index selections where each "index selection" can be:<br/>- a column index or comma-delimited indices<br>- an array with one index, meaning that index and all higher indices<br>- an array with two comma-delimited indices, meaning all the columns in the indices range exclusive of the ending index<br/> default: capture all columns (`[]`) | Use to:<br/>- Select the columns you want to output using zero-based column indices or indices ranges. <br>- Specify to treat *unselected* columns as row labels.  Each selected column can use the text in unselected columns as anchors for its fields. For an illustration, see section-nuances#column-selection TODO LINK.<br/>For example, in a 7-column table `[[0,5]]` selects 1st through 5th columns, and treats 7th column as row labels. <br/>`[1,3,-1]`  selects 2nd, 4th, and the second-to-last columns. Use negative indices to offset from the last element of the column array which is `-1`. <br/> `[1,[3,7]]` selects the 2nd columns and 4th through 8th columns .<br/>  `[[1]]` selects the 2nd column and all columns after it.<br/><br/> For more information, see the Examples section. TODO specify which examples now that they link out |
