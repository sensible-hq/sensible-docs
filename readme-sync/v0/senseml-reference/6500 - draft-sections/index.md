...
| requireStop           | Boolean. default: false                                      | If true, the Stop parameter is required, and the section ends when it matches the Stop parameter, instead of the default behavior of ending at the next starting line specified in the anchor's Match parameter. For example, use this parameter when the starting line repeats in the section, to avoid ending the section before it completes. **Note**  Configure this parameter only if the multiple matches follow each other *vertically* on the page. You don't need to configure this parameter if the multiple matches are distributed along one *horizontal* line in the section (in such a case, Sensible ignores any zero-height sections generated by this horizontal line's matches). |
