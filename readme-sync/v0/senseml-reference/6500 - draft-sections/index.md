...
Use `"direction":"vertical"` to define a "vertical section" in which to find text in column-like layouts. For example, use vertical sections to extract tables nested in tables, tables with row labels, or other complex text layouts. 
...
| lineSelection   | Match object, or array of Match objects         | Use to include lines that span columns in each column and that otherwise break column recognition. For example, use this to include a footnote that is applicable to each column. Each column then becomes irrgularly shaped in order to include the selected lines. |
| lineFilters     | Match object, or array of Match objects         | Use to remove lines that span columns and break column recognition. For example, if the lines occur mid-column, use this parameter rather than an offset parameter to exclude the lines. |
