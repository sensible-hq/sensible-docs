...
| Single LLM Completion               | boolean. default: false                                      | Expects output to be less than the model output token limit. TODO | TODO --- what does it to do chunk overlap??? it sets it to zero and ignores any configured value, right? |
| confidenceSignals                   | boolean.<br/>defaults: true in  visual editor,<br/> false in JSON editor | If specified, Sensible prompts the LLM to report any uncertainties it has about the accuracy of its response.  For more information, see [Qualifying LLM accuracy](doc:confidence).<br/>Applies to each supported method in the config. Sensible currently supports this parameter for the [Query Group](doc:query-group) method. |                                                              |
|                                     |                                                              | ***MISCELLANEOUS***                                          |                                                              |
