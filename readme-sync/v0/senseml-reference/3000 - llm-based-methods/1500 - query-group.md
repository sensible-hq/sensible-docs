...
| confidenceSignals      | boolean.<br/>default: false | If true, Sensible prompts the LLM to report any uncertainties it has about the accuracy of its response.  For more information, see [Qualifying LLM accuracy](doc:confidence). | Not supported if you set the Multimodal Engine parameter on the Query Group method. |
|                        |                                                              | ***FIND CONTEXT***                                           |                                                              |
