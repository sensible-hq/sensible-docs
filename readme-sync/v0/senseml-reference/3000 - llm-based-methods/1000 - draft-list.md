...
|                            |                                                  | ***CONTEXT COMPLETNESS***                                    |                                                              |
| llmEngine                  | `fast`, `thorough`, `long`. <br/>default: `fast` | Configures the LLM model to which Sensible submits the full prompt. Contains the following parameters:<br/>`provider`:  If set to `open-ai` (default), Sensible uses Open AI's GPT-4 model where not hard coded. See the Notes section for more information.  <br/> - If set to `anthropic`, Sensible uses Anthropic's Claude model where not hard coded. See the Notes section for more information. <br/>`mode`: <br/>- `fast`:  Sensible uses a faster LLM model (GPT-4o mini or (Claude 3.5 Haiku) . <br/>- `thorough`: Sensible uses a slower LLM model (GPT-4o or Claude 3.5 Sonnet)  Sensible can take several minutes to return the list.  Use this option if the Fast parameter results in incomplete extractions for multi-page lists.<br/>- `long`: Sensible uses a faster LLM model (GPT-4o mini or Claude 3.5 Haiku). If you set this value, then Sensible can output lists extracted from up to 100 potentially nonconsecutive, 1-page source [chunks](doc:list#notes). Otherwise Sensible by default extracts from 20 1-page chunks. If the list in the document is longer than the number of source chunks, Sensible truncates the list.<br/>For more information, see [Notes](#notes). |                                                              |
