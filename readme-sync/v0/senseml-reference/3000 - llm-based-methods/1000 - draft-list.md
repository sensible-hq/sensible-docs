...
- Sensible can extract up to ~16 tokens (about 20 pages) for a single list field if you set the LLM Engine parameter to fast or thorough, and up to ~40k-50k tokens (about 100 pages) if you set this parameter to long. If the extracted list exceeds the limit, Sensible truncates the list.
...
| llmEngine                  | `fast`, `thorough`, `long`. <br/>default: `fast` | Specifies the LLM model to which Sensible submits the full prompt.<br/>- `fast`:  Sensible uses a faster LLM model (GPT-4o mini) . <br/>- `thorough`: Sensible uses a slower LLM model (GPT-4o)  Sensible can take several minutes to return the list.  If the Fast parameter results in incomplete extractions for multi-page lists.<br/>- `long`: Sensible uses a faster LLM model (GPT-4o mini). If you set this, then Sensible can output lists up to ~40-50k tokens long (corresponding to source lists that occupy about 100 potentially nonconsecutive pages), instead of a default output limit of about ~16k tokens (correspond to source lists that occupy about 20 potentially nonconsecutive pages).<br/>For more information, see [Notes](#notes). | If you specify Fast for this parameter, Sensible can submit a smaller number of chunks than specified by the Chunk Count parameter.<br/>If you specify Long for this parameter, Sensible sets the Chunk Count parameter to 100, so that Sensible uses 100 source pages as context. |
| singleLLMCompletion        | boolean. default: false                          | If Sensible returns incomplete or duplicate results in a multi-page list, set this parameter to True to troubleshoot.  If true, Sensible concatenates the top-scoring chunks into a single batch to send as context to the LLM, instead of batching the chunks into groups. By avoiding splitting context, you can avoid problems such as the LLM failing to recognize the end of one list and the start of another. See Notes for more information.<br/><br/>- The following limits apply:<br/>&nbsp;&nbsp; - If the extracted list exceeds ~16k tokens, Sensible truncates the list.<br/>&nbsp;&nbsp;- Rarely, the extracted list can exceed a ~16k token output limit if the chunks that form its [context](doc:list#notes) have a very small font size. In that case Sensible returns an error. To avoid this type of error, set this parameter to false. | Don't set this parameter to true if you set the LLM Engine parameter to Long. This parameter truncates list output to 16k tokens (typically about 20 pages), so it'll truncate long lists. |
...
3. To avoid large language model (LLM)'s token limits, Sensible batches the chunks into groups. The chunks in each page group can come from non-consecutive pages in the document. If you set the Single LLM Completion parameter to True, then Sensible creates a single group that contains all the top-scoring chunks and sets a larger maximum token limit for the single group (about 120k tokens) than it does for batched groups.
