...
- Sensible can extract up to 20 pages for a single list field if you set the LLM Engine parameter to fast or thorough, and up to 100 pages if you set it to long. If the list exceeds the limit, Sensible truncates the list.
...
3. To avoid large language model (LLM)'s token limits, Sensible batches the chunks into groups by page numbers. The chunks in each page group can be non-consecutive in the document. If you set the Single LLM Completion parameter to True, then Sensible creates a single page number group that contains all the top-scoring chunks and sets a larger maximum token limit for the single group (about 120k tokens) than it does for batched groups.
