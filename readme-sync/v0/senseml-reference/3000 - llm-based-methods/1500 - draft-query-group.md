...
| source_ids             | array of field IDs in the current config | If specified, Sensible prompts an LLM to search for the answers to the queries in the group in the output of the fields listed in this parameter.  The output of the source field(s) is the [context](doc:prompt#notes) that Sensible provides to the LLM. Use this parameter to reformat or otherwise transform the outputs of other fields. You can also use this field to troubleshoot or simplify complex prompts that aren't performing reliably. Break the prompt into several parts, and chain it into successive instructions by using a source Query Group field, then specifying additional instructions with that field as the Source IDs for the next field. <br/>For example,  if you extract an inconsistently formatted ranking (`first`, `1st`,  `1`) in a `top_selling` field, then  you can specify the `top_selling` field in this parameter in order to prompt for additional formatting, e.g., `format each inconsistently formatted instance of rank as a word (i.e. \"first\" not \"1st\" or \"1\")`  <br/> If the source fields contain repeating data (for examples, lists or sections), then generally it's best to prompt for single data points from those repeating data, e.g., "what's the largest transaction?" or "which date were the most transactions made"?<br/>For an example, see TBD/2do. | If you configure this parameter, then in the query group:<br/>- Sensible doesn't support confidence signals.<br/>- Sensible doesn't allow you to specify the following parameters:<br/>- Multimodal Engine parameter <br/> Chunk Scoring Text parameter<br/>Search By Summarization parameter<br/>Page Hinting parameter<br/>Chunk Count parameter<br/>Chunk Size parameter<br/>Chunk Overlap Percentage parameter<br/>Page Range parameter |
