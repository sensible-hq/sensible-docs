...
| source_ids             | array of field IDs in the current config | If specified, Sensible prompts an LLM to search for the answers to the queries in the output of the fields listed in this parameter. In other words,  the output of the source field(s) is the [context](doc:prompt#notes) that Sensible provides to the LLM.<br/>Use this parameter to reformat or otherwise transform the outputs of other fields. You can also use this field to troubleshoot or simplify complex prompts that aren't performing reliably. Break the prompt into several simpler parts, and chain it into successive instructions by using a source Query Group field, then specifying additional instructions with that field specified in the Source IDs parameter for the next field. <br/> If the source fields contain repeating data (for examples, lists or sections), then generally it's best to prompt for single data points from those repeating data, e.g., "what's the largest transaction?" or "which date were the most transactions made"?<br/>For an example, see TBD/2do. | If you configure this parameter, then in the query group:<br/>- Sensible doesn't support confidence signals.<br/>- Sensible doesn't allow you to specify the following parameters:<br/>- Multimodal Engine parameter <br/> Chunk Scoring Text parameter<br/>Search By Summarization parameter<br/>Page Hinting parameter<br/>Chunk Count parameter<br/>Chunk Size parameter<br/>Chunk Overlap Percentage parameter<br/>Page Range parameter |
