...
| :------------------------- | :--------------------------------------- | :----------------------------------------------------------- |
| id (**required**)          | `zip`                                    |                                                              |
| source_ids  (**required**) | array of field IDs in the current config | Depending on the intended zip, one of the following: <br/><br/>- the IDs of the fields to zip together.<br/><br/>- a single ID for a field that returns a table. If you mix array source IDs with one or more table source IDs, the Zip method zips the first table and ignores everything else (tables take precedence over arrays for zipping).<br/><br/>- the IDs of the sections to zip together. If you mix section source IDs with non-section IDs, the Zip method ignores the non-section IDs and outputs the sections.<br/><br/> If the output of the source IDs are arrays (for example, if you specify `"match: all"`, or if the source IDs are sections), the Zip method joins them up to their maximum shared length. For example, if you zip arrays that have 4, 5, and 6 elements respectively, the zipped array has 4 elements. |
