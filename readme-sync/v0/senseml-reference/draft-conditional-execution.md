...
- *fallbacks topic: add info on 'conditional execution' framed as a more sophisticated form of falling back/alternative to 'fallback configs'?*
...
| ----------------------------------------------------- | ----------------------------------------- | ------------------------------------------------------------ |
| id (**required**)                                     | `conditional`                             | Extracts alternate lists of fields based on a pass/fail condition. You can nest conditional methods inside conditional methods to any depth. Fields can [fallback](doc:fallback) across conditional methods at any depth. For example, a nested `checking_transactions` field can fallback from a nested conditional method to a `checking_transactions` field in the top-level `fields` object, and vice versa. |
