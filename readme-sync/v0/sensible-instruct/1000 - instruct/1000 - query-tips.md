...
| Answer might not fully answer the prompt | The LLM answers the prompt, but is uncertain whether the context that Sensible provides to the LLM contains the full answer. | - Simplify your prompt, for example, break it up into multiple questions.<br/>- See [Advanced prompt configuration](doc:prompt). |
