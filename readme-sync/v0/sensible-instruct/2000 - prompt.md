...
| promptIntroduction                   | string.                                                      | Overwrites the default text at the beginning of the full prompt that Sensible submits to the LLM. <br/> This parameter applies to each method of a specified type in a config.<br/>For more information and for an example, see [NLP](doc:nlp) preprocessor. Note that each method has its own prompt introduction default.<br/>**Note**: If you set the Confidence Signals parameter to true, this parameter is not editable. Sensible uses the default Prompt Introduction text, and appends instructions about confidence signals. For more information, see the Confidence Signals parameter. | Overrides config-level parameter for a single field. <br/> For more information, see the Sensible Instruct  reference topic for the method you want to edit, for example,  [List](doc:list#parameters).  Note that each method has its own prompt introduction default.<br/>TODO add links to examples?? |
| ***hinting parameters***             |                                                              |                                                              |                                                              |
...
| ***confidence signals parameters***  |                                                              |                                                              |                                                              |
| confidenceSignal                     | boolean. default: true                                       | See Confidence Signals for more information. TODO link. <br/>If this is true, then you can't edit the default text for the promptIntroduction.<br/>When this is true, Sensible appends the following text to the default promptIntroduction for the Query method:<br/>`if the answer is not contained within the text below, return '{"answer": "I don't know", "uncertainties": ""}'. Further, do your best to list the uncertainties in answering the question from the following: ("Multiple candidate answers", "Answer might not fully answer question", "Answer not found in the context", "Ambiguous query"). Finally, return the output in JSON format for example: '{"answer": "...", "uncertainties": "Answer might not fully answer question"}` |                                                              |
